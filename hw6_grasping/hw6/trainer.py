...
class MobileUNet(nn.Module):
    def __init__(self, pretrained: bool=True, num_rots: int=2) -> None:
        ...
        self.upsample1_1 = nn.Upsample(scale_factor=4, mode="bilinear", align_corners=True)
        self.conv1_1 = nn.Conv2d(24, 4, 1)

        self.conv2_1 = nn.Conv2d(40, 128, 1)
        self.upsample2_1 = nn.Upsample(scale_factor=8, mode="bilinear", align_corners=True)
        self.conv2_2 = nn.Conv2d(128, 4, 1)

        self.conv_com = nn.Conv2d(4, 2, 1)
        self.upsample_com = nn.Upsample(scale_factor=2, mode="bilinear", align_corners=True)
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        x_w = self.upsample1_1(x_wide)
        x_w = self.conv1_1(x_w)

        x_n = self.conv2_1(x_narrow)
        x_n = self.relu(x_n)
        x_n = self.upsample2_1(x_n)
        x_n = self.conv2_2(x_n)

        x = x_n + x_w
        x = self.conv_com(x)
        x = self.relu(x)
        x = self.upsample_com(x)
